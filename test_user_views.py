...
class MessageViewTestCase(TestCase):
    ...
    def setup_likes(self):
        m1 = Message(text="warble", user_id=self.testuser_id)
        m2 = Message(text="another warble", user_id=self.testuser_id)
        m3 = Message(id=9876, text="yet another warble", user_id=self.u1_id)
    ...
    def test_add_like(self):
        m = Message(id=1984, text="Hello", user_id=self.u1_id)
        db.session.add(m)
        db.session.commit()
        with self.client as c:
    ...
    def test_remove_like(self):
        self.setup_likes()
        m = Message.query.filter(Message.text=="another warble").one()
